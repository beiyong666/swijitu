export async function onRequestGet({ request, env, params }) {
  ...
  if(maybe){
    if(!wj) return new Response(JSON.stringify({ ok:false, error:'KV binding wj not found' }), { status:500, headers: { 'content-type': 'application/json; charset=UTF-8', 'Access-Control-Allow-Origin': '*' } });
    const key = 'dir:' + maybe;
